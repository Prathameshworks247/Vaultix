...
def create_payment(
    ...
    db.add(payment)
    db.add(PaymentEvent(payment=payment, event_type="CREATED", detail={"amount": str(body.amount)}))
    try:
        db.commit()
...
def get_payment(request: Request, payment_id: UUID, db: Session = Depends(get_db)):
    ...
    return {
        **PaymentOut.model_validate(p).model_dump(),
        "events": [{"type": e.event_type, "at": e.created_at, "detail": e.detail} for e in p.events],
    }
