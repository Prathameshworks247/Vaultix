        ...
        try:
            ...
            if random.random() < RECEIPT_FAILURE_RATE:
                ...
                if self.request.retries >= self.max_retries:
                    ...
                    db.commit()
                    logger.error("receipt email failed: retries exhausted")
                    return payment_id
                logger.warning(f"simulated SMTP failure on attempt {self.request.retries + 1}, retrying")
                raise self.retry(
            ...
            logger.info(f"receipt sent: amount={payment.amount} {payment.currency}")
    ...
    with bind_payment_id(payment_id):
        try:
            ...
            if not merchant.webhook_url:
                ...
                logger.info(f"merchant {merchant.id} notified (no webhook registered): status={payment.status}")
            ...
            try:
                ...
                if self.request.retries >= self.max_retries:
                    ...
                    db.commit()
                    logger.error(f"webhook delivery failed, giving up: {exc}")
                    return payment_id
                logger.warning(f"webhook delivery attempt {self.request.retries + 1} failed, retrying: {exc}")
            ...
            db.commit()
            logger.info(f"webhook delivered: status_code={resp.status_code}")
            return payment_id
        finally:
            db.close()
